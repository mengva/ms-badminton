...
import { GlobalSecureFileUploadServices } from '@/server/packages/utils';
...
export class SecureFileUploadServices {
    ...
    private static async validationFileAndUploadFunc(file: FileDto, callback: (file: FileDto) => Promise<CLOUDINARY_DTO | any>): Promise<ResponseFileDto> {
        try {
            const validationFile = GlobalSecureFileUploadServices.validationFile(file);
    ...
    public static async uploadCloudinaryImageFile(file: FileDto): Promise<ResponseFileDto | undefined> {
        try {
    ...
    public static async uploadCloudinaryPDFFile(file: FileDto): Promise<ResponseFileDto | undefined> {
        try {
            if (!file) return undefined;
    ...
    public static async uploadCloudinaryPDFFiles({ files }: ImageFileDto): Promise<ResponseFileDto[]> {
        try {
            if (!files.length) return [];

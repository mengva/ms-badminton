...
export const rentalPlanEnum = pgEnum("rental_plan", [
  "Monthly",
  "Quarterly",   // 3 ເດືອນ
  "SemiAnnual",  // 6 ເດືອນ
  "NineMonths",  // 9 ເດືອນ
...
export const users = pgTable("users", {
  ...
  birthDay: varchar("birth_day", { length: 20 }).notNull(),
...
export const contracts = pgTable("contracts", {
  ...
  rentalPlan: rentalPlanEnum("rental_plan").notNull(),
  ...
  totalRentAmount: numeric("total_rent_amount", { precision: 12, scale: 2, }).notNull(),
...
export const checkIns = pgTable("check_ins", {
  ...
  roomConditionNote: text("room_condition_note").notNull(),
  note: text("note"),
  createdAt: timestamp("created_at", { withTimezone: true }).defaultNow().notNull(),
  updatedAt: timestamp("updated_at", { withTimezone: true }).defaultNow().notNull().$onUpdateFn(() => new Date()),
}, (table) => [
...
export const roomTransfers = pgTable("room_transfers", {
  ...
  reason: text("reason"),
  priceDifference: numeric("price_difference", { precision: 12, scale: 2 }).notNull(),
  note: text("note"),
  createdAt: timestamp("created_at", { withTimezone: true }).defaultNow().notNull(),
  updatedAt: timestamp("updated_at", { withTimezone: true }).defaultNow().notNull().$onUpdateFn(() => new Date()),
}, (table) => [
...
export const utilityBills = pgTable("utility_bills", {
  ...
  billingMonth: varchar("billing_month", { length: 10 }).notNull(), // "2025-01"
  // ໄຟຟ້າ
...
export const invoices = pgTable("invoices", {
  ...
  roomId: uuid("room_id").references(() => rooms.id, { onDelete: "cascade" }).notNull(),
  // ເດືອນບິນ
  billingMonth: varchar("billing_month", { length: 10 }).notNull(), // "2026-01"
  // 💰 ລາຍລະອຽດເງິນ
  rentAmount: numeric("rent_amount", { precision: 12, scale: 2 }).default("0"),
  utilityAmount: numeric("utility_amount", { precision: 12, scale: 2 }).default("0"),
  otherAmount: numeric("other_amount", { precision: 12, scale: 2 }).default("0"),
...
export const roomsRelations = relations(rooms, ({ one, many }) => ({
  owner: one(users, { fields: [rooms.ownerId], references: [users.id], relationName: "roomOwner" }),
...
export const invoicesRelations = relations(invoices, ({ one }) => ({
  ...
  customer: one(users, { fields: [invoices.customerId], references: [users.id] }),
  staff: one(users, { fields: [invoices.staffId], references: [users.id] }),
